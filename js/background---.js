...
function gotStream(stream) {
    ...
    console.log(audioStream.getAudioTracks().length);
...
function stopScreenRecording() {
    ...
    recorder.stopRecording(function() {
        var file = new File([recorder.getBlob()], 'VideoRecording-' + (new Date).toISOString().replace(/:|\./g, '-') + '.webm', {
            type: 'video/webm'
        });
...
function getUserConfigs() {
    chrome.storage.sync.get(null, function(items) {
        ...
        if (items['recording_type'] && items['recording_type']=="tab") {
            recording_type = "tab";
        }else{
            recording_type = "desktop";
        }
        console.log(recording_type);
...
chrome.tabs.onActivated.addListener(function(activeInfo) {
    chrome.tabs.get(activeInfo.tabId, function (tab) {
        if(tab.url.indexOf("chrome://")==-1){
            chrome.storage.sync.get("isRecording", function(data) {
                if(data.isRecording=="yes"){
                    if(recording_type=="desktop"){
                        chrome.storage.sync.get("webcam", function(data) {
                            if(data.webcam){
...
chrome.tabs.onUpdated.addListener(function(tabid, changeinfo, tab) {
    ...
    if (url !== undefined && changeinfo.status == "complete") {
        if(tab.url.indexOf("chrome://")==-1){
            chrome.storage.sync.get("isRecording", function(data) {
                if(data.isRecording=="yes"){
                    if(recording_type == "tab"){
                        if(tab.id==current_tab_recording){
                            chrome.tabs.insertCSS(null, {
                                file: "css/jquery-ui.css"
                            });
                            chrome.tabs.insertCSS(null, {
                    ...
                    }else if(recording_type=="desktop"){
                        chrome.tabs.insertCSS(null, {
                            file: "css/jquery-ui.css"
                        });
                        chrome.tabs.insertCSS(null, {
...
chrome.extension.onMessage.addListener(function (request, sender, sendResponseParam) {
    if (request.type == "tab") {
        chrome.storage.sync.get("webcam", function(data) {
          ...
          if(data.webcam){
            ...
            chrome.tabs.query(
                { currentWindow: true, active: true },
                function (tabArray) { 
                    current_tab_recording = tabArray[0].id;
                }
            );
            setTimeout(function(){ getUserConfigs(); }, 3000);
          }else{
            getUserConfigs();
          }
        });
        sendResponseParam({status: "started"});
        ...
        current_tab_recording = "";
        getUserConfigs();
        sendResponseParam({status: "stopped"});
    }else if(request.action=="status"){
        console.log(isRecordingVOD);
        sendResponseParam({status: isRecordingVOD});   
    }
});